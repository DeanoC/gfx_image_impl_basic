...
#include "gfx_image/block.h"
#include "gfx_imageformat/formatcracker.h"

AL2O3_EXTERN_C void Image_BlockDecodeColor(
    uint8_t *dest,
    int blockWidth, int blockHeight,
    int pixelPitch, int rowPitch, enum ImageFormat format, int red, int blue, uint8_t const *src) {
  uint8_t colors[4][3];
  ...
  if (c0 > c1 ||
      ((format == ImageFormat_BC3_SRGB_BLOCK) ||
          (format == ImageFormat_BC3_UNORM_BLOCK))) {
    for (int i = 0; i < 3; i++) {
...
AL2O3_EXTERN_C void Image_BlockDecodeBC1(uint8_t *dest, uint8_t const *src) {
  Image_BlockDecodeColor(dest, 4, 4, 3, 3 * 4, ImageFormat_BC1_RGB_UNORM_BLOCK, 0, 2, src);
}

AL2O3_EXTERN_C void Image_BlockDecodeBC2(uint8_t *dest, uint8_t const *src) {
  Image_BlockDecodeColor(dest, 4, 4, 4, 4 * 4, ImageFormat_BC2_UNORM_BLOCK, 0, 2, src);
...
AL2O3_EXTERN_C void Image_BlockDecodeBC3(uint8_t *dest, uint8_t const *src) {
  Image_BlockDecodeColor(dest, 4, 4, 4, 4 * 4, ImageFormat_BC3_UNORM_BLOCK, 0, 2, src);
...
AL2O3_EXTERN_C void Image_BlockDecodeCompressedData(uint8_t *dest,
  ...
  ASSERT(ImageFormat_IsCompressed(format));

  int blockWidth = (width < 4) ? width : 4;
  int blockHeight = (height < 4) ? height : 4;

  int nChannels = ImageFormat_ChannelCount(format);
...
AL2O3_EXTERN_C bool Image_BlockDecodeIsSupported(ImageFormat format) {
  switch (format) {
    case ImageFormat_BC1_RGB_SRGB_BLOCK:
    case ImageFormat_BC1_RGB_UNORM_BLOCK:
    case ImageFormat_BC1_RGBA_SRGB_BLOCK:
