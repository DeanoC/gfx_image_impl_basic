...
AL2O3_EXTERN_C void Image_CopyImageChain(Image_ImageHeader const *dst,
...
AL2O3_EXTERN_C Image_ImageHeader *Image_PreciseConvert(Image_ImageHeader *image, ImageFormat const newFormat) {
  Image_ImageHeader *dst = Image_Create(image->width, image->height, image->depth, image->slices, image->format);
  if (dst == nullptr) { return nullptr; }
  Image_CopyImage(dst, image);
  if (image->nextType != Image_IT_None) {
